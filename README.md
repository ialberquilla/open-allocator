...
[![Tests](https://img.shields.io/badge/tests-819%20passing-brightgreen.svg)](#development)
...
**It does not pick your trade-off for you.** Independence and yield pull against each other, so the allocator exposes both controls and *measures* what your choice cost — per run, on the shelf as it stands that day. Want the top rates? That is a knob. Want the most independent book the shelf allows? Also a knob.
...
```bash
git clone https://github.com/ialberquilla/open-allocator && cd open-allocator
uv sync
...
Create an account and generate a key at **[app.1tx.fi/settings](https://app.1tx.fi/settings)**.
...
Set `ONE_TX_API_KEY` and you are done — [.env.example](.env.example) walks the rest as numbered steps, each one gated by a condition so you can skip any block that does not apply. The defaults (`eoa` + `rpc` + `local`) need nothing but that key and a funded private key. See [Configuration](#configuration) for the shape of the choices.

**4 — Look around, read-only**
...
Neither command signs anything. `list-vaults` needs only the API key; `wallet-status` is the check that your signer config resolves.

**5 — Point an agent at the repo**

You are not meant to type these commands. Open the repo in a coding agent (Claude Code, Cursor, or anything that can run a shell) — it reads [AGENT_GUIDE.md](AGENT_GUIDE.md) and the [skills](#agent-operation) — and drive it in plain language from there. Every spend stays confirmation-gated.
...
**[.env.example](.env.example) — credentials and how you sign.** Copy it to `.env` and read it top to bottom: it is written as numbered steps, and every step past the third carries the condition that makes it apply. Steps 1–3 are the whole setup for the default. The only structural choice is three independent axes, each of which points at the step that explains it:

| Axis | Values | Question it answers |
| --- | --- | --- |
| `SIGNER_ACCOUNT` | `eoa` (default) · `safe` | What holds the funds |
| `SIGNER_SUBMISSION` | `rpc` (default) · `erc4337-paymaster` | How the transaction reaches the chain |
| `SIGNER_OWNER` | `local` (default) · `remote` | Where the signing key lives |

Leave all three alone and you need a 1Tx API key and a funded private key. Set `safe` + `erc4337-paymaster` and you get [gas in USDC](#gas-in-usdc-no-native-tokens). Secrets are read from the OS keychain *before* `.env`, so any value can live there instead; `OPEN_ALLOCATOR_ENV_FILE` points the loader outside the working tree if you would rather no plaintext file sits where an agent can read it.

**[policy.yaml](policy.yaml) — the allocator's constitution.** It defines `allowed` axes (protocols, chains, `asset_categories`, `stablecoin_only`, assets, curators), `caps` (per-instrument / protocol / curator / chain / sector weight, min TVL, max reward dependence, and `min_effective_positions` — the measured floor, which fails closed), and `gates` (new-instrument approval, autonomous rebalance, max deploy per cycle). Allowlists are narrowing filters over discovery (`null` = all); they never replace discovery.
...
Reference notes in [docs/](docs/): [capabilities](docs/capabilities.md) (full knob surface and metric limits), [gasless-execution](docs/gasless-execution.md), [funding-and-bridging](docs/funding-and-bridging.md), [safe-guard-policy](docs/safe-guard-policy.md), [onetx-analysis-fields](docs/onetx-analysis-fields.md).
...
```bash
uv run ruff check
uv run pytest            # 819 passed, 3 integration tests skipped without live creds
```
